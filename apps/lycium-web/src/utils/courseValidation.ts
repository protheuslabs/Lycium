...
type CourseValidationOptions = {
  centralSourceRecords?: ReadonlyArray<SourceRecordLike>;
...
function getDeclaredSourceIds(course: CourseData, centralSourceRecords: ReadonlyArray<SourceRecordLike>): Set<string> {
  ...
  if (Array.isArray(courseSourceRecords)) {
    for (const source of courseSourceRecords) {
      if (source && typeof source === "object" && isNonEmptyString((source as SourceRecordLike).id)) {
        declared.add((source as { id: string }).id);
      }
    }
...
function getQuizQuestions(block: CourseBlock): unknown[] {
...
function validateConceptCards(block: CourseBlock, location: string, errors: string[]) {
...
function validateQuizBlock(block: CourseBlock, location: string, errors: string[]) {
...
function validateSection(
  ...
  if (quizBlocks.length > 0) {
    ...
    quizBlocks.forEach((block, quizIndex) => validateQuizBlock(block, `${sectionLocation} quiz ${quizIndex + 1}`, errors));
    return;
  }
...
export function validateCourseEntry(
