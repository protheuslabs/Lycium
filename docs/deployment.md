...
- Public URL root: `https://protheuslabs.github.io/Lycium/`
- Base path: `NEXT_PUBLIC_LYCIUM_BASE_PATH=/Lycium`
- Build mode: `NEXT_OUTPUT=export`
- Runtime: `NEXT_PUBLIC_LYCIUM_RUNTIME=static`
- Artifact directory: `apps/lycium-web/out`
- Direct-route fallback: the workflow copies `out/index.html` to `out/404.html`

The static export artifact is generated by the Pages workflow and is ignored locally through `out` in `.gitignore`.
...
- Keep local API keys and provider settings in ignored local data, not in repository files.
