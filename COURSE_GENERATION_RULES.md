...
- Course template generation is the first active course-content workflow.
  It emits an inspectable `course-template-v1` artifact plus `course-template-quality-report-v1` before module planning. The template should resolve the course title, short description, scope, target audience, level, duration, outcome, assessment expectations, learning outcomes, source packet/source ID handoff, and a `course-coverage-checklist-v1`. It must not create modules, sections, build tasks, or learner-facing content.
...
- Before fallback or under-sourced module planning, generate a course-level `course-coverage-checklist-v1` that names the required curriculum topics for the course.
  Module planning must assign every required coverage item to a module, and section planning must assign module coverage items to empty section shells before section fill starts.
- Module section planning is a separate active workflow from module outline generation and section fill.
  It expands one module outline into section-plan records and adds planned empty section shells to the course/module structure with section titles, planning-only descriptions, learning objectives, concept keywords, candidate source IDs in metadata, and empty `content` arrays before section fill starts.
...
- Course, module, and section content-fill runs should be thin orchestrators over section fill.
  They may loop through planned empty sections, save progress after each section, generate module Apply/Summary artifacts after lesson sections are filled, and support scoped retries, but they should not bypass the section-fill workflow with a separate content writer.
- Module Apply generation is a separate active workflow after section fill and before module assembly.
  It should orchestrate assessment planning, routed quiz/test generation, routed project generation, and final Apply-section validation rather than writing assessment content directly. The assessment plan should inspect filled lesson content plus course/module requirements to decide whether the Apply section should be a quiz/test or project unless a requirement specifies the assessment type.
- Module quiz/test generation and module project generation are separate sub-workflows.
  Quiz/test generation must create realistic questions that make learners calculate, classify, interpret, predict, or apply taught ideas. It must not use generic meta-prompts such as "Which answer best demonstrates mastery of X?", "memorize the label", or "assess an unrelated idea." Quiz-based module assessments need at least 10 valid questions, with larger banks for unit tests or final-style checks. Project generation should create applied tasks with instructions, required evidence, rubric criteria, one canonical submission type, and grader workflow metadata.
  Apply sections should omit section/block `sourceIds` so they do not render a source footer, because they assess the course content itself. Empty assessment payloads should fail instead of being hidden in module assembly.
- Assessment plans should record coverage variables before routed generation starts.
  Required fields include assessment kind, assessment scale, coverage scope, minimum content coverage ratio, target section IDs, target concept IDs, target coverage item IDs when available, and inherited quiz/project specs. Default minimum content coverage is 70% for both quizzes/tests and projects. Regular module checks default to current-module coverage, unit tests may cover the current and previous modules, and final-style Apply sections should cover the entire course unless requirements say otherwise. Generated Apply sections should persist the compact plan in `metadata.assessmentPlan` so a bad quiz/project, module, or whole course can be rebuilt later from the same coverage scope.
- Quiz/test specs should inherit from the assessment plan and may define question count, time limit, question types, and multiple-answer ratio. The default multiple-answer ratio is 0. Project specs should inherit from the assessment plan and define submission type, required evidence expectations, rubric criteria, and grader workflow metadata.
...
- `course_template_generation`: resolve the first active handoff artifact with title, catalog description, scope, learning outcomes, source packet/source ID handoff, coverage checklist, desired module count, and next workflow pointer. This gate validates the handoff while guaranteeing there are no modules, sections, build tasks, or learner-facing content yet.
...
- `coverage_checklist`: create or preserve the required concept inventory for the course, record it in `metadata.courseCoverageChecklist` when no stronger benchmark-derived outline exists, and verify every required item is assigned to at least one module and section through `metadata.courseCoverageAllocationReport`.
...
- `assessment`: create mastery-evidence sections that test or verify previously taught or sourced concepts. First run assessment planning to choose quiz/test versus project/lab/portfolio evidence unless requirements specify the assessment type. The plan must record coverage scope and a minimum content coverage ratio, defaulting to 70%. Evidence may be a quiz, longer test/exam-style question bank, project, lab, simulation, portfolio task, or rubric-graded submission. Quiz sections remain quiz-only and should ask realistic content questions, not generic "mastery of concept" meta-questions.
...
6. Find citations and sources for each idea.
   ...
   If submitted sources do not meet the course's source coverage and source-strength policy, create or preserve a `needs_sources` draft with structured `metadata.sourceGaps`, a real module and section outline, and empty planned section shells whose descriptions, outcomes, concepts, source needs, and candidate source IDs live in planning metadata for later section fill. Source readiness is based on concept coverage, depth, relevance, authority, extractability, and diversity, not on a simple minimum source count. Keep workflow labels such as `needs_sources` and `source gap` out of learner-facing titles.
...
- `metadata.courseCoverageChecklist`: optional `course-coverage-checklist-v1` curriculum inventory used as the course-to-module-to-section handoff when benchmark or source-packet evidence is not strong enough to define the skeleton on its own.
- `metadata.courseCoverageAllocationReport`: optional `course-coverage-allocation-report-v1` showing whether required coverage items were assigned to modules and planned sections.
...
- Generated courses must use the same atomic block format as the course editor: `text`, `heading`, `conceptCard`, `equation`, `workedExample`, `image`, `visual`, `video`, `iframe`, `quiz`, and `project`. Use `equation` for standalone formulas or symbolic relationships. Use `workedExample` only for quantitative, formal, technical, or procedural problems where learners should see repeatable work; do not use it for interpretive humanities/social-science analysis, historical causation, literary interpretation, ethics discussion, or source-analysis prompts. Project blocks may include nested rubric, submission, and grader workflow objects. Do not generate monolithic markdown, plain-string section content, or large nested concept-card payloads when smaller editable blocks are possible.
