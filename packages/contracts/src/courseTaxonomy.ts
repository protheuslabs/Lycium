...
export function isCourseCategoryId(categoryId?: string) {
  return Boolean(categoryId && categoryLabels.has(categoryId));
}

export function isCourseDepartmentInCategory(categoryId?: string, departmentId?: string) {
  if (!categoryId || !departmentId) {
    return false;
  }

  return getCourseCategoryDepartments(categoryId).some((department) => department.id === departmentId);
}

export function getCourseDepartmentLabel(categoryId?: string, departmentId?: string) {
  if (!departmentId) {
    return "Not set";
  }

  const department = getCourseCategoryDepartments(categoryId).find((item) => item.id === departmentId);
