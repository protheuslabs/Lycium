...
def test_needs_sources_snapshot_builds_generation_readiness_when_omitted() -> None:
    ...
    assert all(section["sectionType"] == "lesson" for section in sections)
    assert all(section["content"] == [] for section in sections)
    assert all(section.get("sourceIds") == [] for section in sections)
    assert all(section["metadata"]["generationOutline"]["contentStatus"] == "planned_empty" for section in sections)
    assert all(section["metadata"]["generationOutline"]["plannedDescription"] for section in sections)
    assert all(section["metadata"]["generationOutline"]["plannedLearningOutcome"] for section in sections)
...
def test_generation_with_weak_source_strength_returns_needs_sources_draft(client) -> None:
    ...
    assert course["metadata"]["generationPlan"]["mode"] == "outline_first_empty_section_plan"
    assert course["metadata"]["generationPlan"]["nextWorkflow"] == "section_fill"
    ...
    assert all(section.get("sourceIds") == [] for section in lesson_sections)
    assert all(section["metadata"]["generationOutline"]["role"] == "section_plan" for section in lesson_sections)
    assert all(section["metadata"]["generationOutline"]["nextWorkflow"] == "section_fill" for section in lesson_sections)
    assert all(section["metadata"]["generationOutline"]["rebuildScopes"] == ["section_plan", "section_content"] for section in lesson_sections)
    assert "practical capability" not in str(course).lower()
    assert "decision note" not in str(course).lower()
